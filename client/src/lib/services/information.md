...
* **보존+보강 & 기본 품질**: 두 분기 모두 명시된 것(개체·고유명사·수치·핵심 요청/피사체)은 보존하되, **명시되지 않은 빈칸은 합리적·도메인(장르) 적합 디테일로 적극 채워(enrich)** 초안보다 확실히 풍부한 출력을 내도록 규칙으로 명시합니다. 초안은 '천장'이 아니라 '씨앗'이며, 빈약한 초안이 빈약한 출력으로 이어지지 않게 합니다(특히 콜드 스타트·중립 tier에서).
* **출력 언어**:
