...
import { z } from 'zod';
import { GeneratePromptRequestType, promptGenerationSchema } from '@/lib/schemas/promptSchema';
import { Modality, MODALITY_DIMENSIONS, prefKey } from '@/lib/services/modality';
...
// ── 모달리티 × 차원별 tier 스니펫 맵 ──────────────────────────────────────────
// 각 스니펫은 '타겟 AI가 따라야 할 제약'으로 생성 프롬프트 내부에 삽입된다.
...
// 차원명 → tier 값 (모달리티마다 차원이 다르므로 가변 키)
...
// 생성형 미디어(image/video/music)용 시스템 프롬프트.
// 핵심: 생성되는 content가 "다른 LLM에게 시키는 지시서"가 아니라, 타겟 생성 AI에
// 그대로 붙여넣어 결과물이 나오는 '묘사형 프롬프트 그 자체'여야 한다.
