...
import { generatePromptResponseSchema } from '../../schemas/promptSchema.js';
...
function getSnippetByTier(tier, snippets) {
...
function buildTierDescription(tiers) {
    const labels = Object.keys(TIER_LABELS)
        .map(k => TIER_LABELS[k][tiers[k]] ?? null)
        .filter((l) => l !== null);
    return labels.length ? labels.join(' · ') : '중립형';
}
function generateRandomVariantTiers(exactTiers, exclude) {
    const keys = ['tone', 'level', 'density', 'creativity'];
    for (let i = 0; i < 15; i++) {
        const variant = {};
        for (const k of keys) {
        ...
        if (diffFromExact && diffFromExclude)
            return variant;
    }
...
function buildConstraintSet(tiers) {
    const lines = [
...
// ── 메인 서비스 함수 ──────────────────────────────────────────────────────────
export const generatePromptCandidates = async (requestData) => {
    ...
    if (prefsResult.data) {
        prefsResult.data.forEach((row) => {
            if (row.category && row.category in prefs) {
                prefs[row.category] = row.weight_score;
            }
        });
    }
    ...
    try {
        ...
        const variantLabels = ['exact', 'variant_a', 'variant_b'];
