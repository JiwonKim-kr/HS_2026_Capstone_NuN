...
export function createMcpServer(userId) {
    ...
    // 1. generate_prompt
    server.tool('generate_prompt', 'Generate personalized prompt candidates from an original draft.', {
    ...
    // 2. submit_feedback
    server.tool('submit_feedback', 'Submit like/unlike feedback for a generated prompt candidate to update user preferences.', {
    ...
    // 3. list_history
    server.tool('list_history', 'List all previous prompt generation sessions for the user.', {}, async () => {
        try {
    ...
    // 4. get_session
    server.tool('get_session', 'Get detailed information and candidates for a specific session.', {
    ...
    // 5. delete_session
    server.tool('delete_session', 'Delete a specific session and its candidates.', {
